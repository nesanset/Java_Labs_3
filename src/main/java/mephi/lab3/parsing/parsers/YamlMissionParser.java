...
public class YamlMissionParser implements MissionParser{
    ...
    public void parse(String content, MissionBuilder missionBuilder) throws MissionParseException{
        try{
            ...
            throw new MissionParseException("Не удалось прочитать YAML");
        }
    }
    ...
    private void addExtensionBlock(String blockName, JsonNode node, MissionBuilder missionBuilder){
        ...
        while (fields.hasNext()){
            ...
            if (valueNode.isValueNode()){
                missionBuilder.addExtensionField(blockName, field.getKey(), valueNode.asText());
            } else {
                missionBuilder.addExtensionField(blockName, field.getKey(), valueNode.toString());
            }
        }
    }
